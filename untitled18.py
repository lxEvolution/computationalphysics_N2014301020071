...
class ENERGY_MAG(object):            # give the plot of energy and magnetization vs temperature
    def __init__(self):
        self.T=list(np.arange(0.5,2.1,0.1))+list(np.arange(2.1,2.5,0.01))+list(np.arange(2.5,5,0.1))
    def calculate(self):
        self.energy=[]
        self.mag=[]
        self.e_var=[]
        ...
        for i in self.T:
            ...
            for j in range(self.n):
                ...
                if j > 20 :
                    ...
                    self.temp_mm=np.mean(self.cmp.system)
        ...
        np.fig=plt.figure(figsize=(12,6))
        ax1=plt.subplot(221)
        ax2=plt.subplot(222)
        ax3=plt.subplot(223)
        ax4=plt.subplot(224)
